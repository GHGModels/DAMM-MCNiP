...
Litter_N = Litter_C/27.6;%external N input into SON pool (e.g.leaf litter, FWD) mg N cm-3 soil hour-1
...
% km_upt_N = km_UPT_C;
%b_UPT_C = 0.1; %intercept, mg cm-3 degree-1
%m_UPT_C = 0.01;%slope, mg cm-3

%Depolymerization kinetic temperature relationship parameters %%from Davidson et al 2012
Vmax_0      = 1.0815E11; %Arrhenius constant mg SOM cm-3 soil hours-1
Ea_up       = 61.77; %activation energy for arrhenius equation, kJ mol-1 
...
%CUE %%CUE is set within the model but can make CUE temperature sensitive using this code if desired
...
%this section will calculate available DOC and DON at reaction site for uptake %%currently equal to standing DOCN pool but can make sensitive to soilM if desired
DOC_rs(i) = DOC(i);
DON_rs(i) = DON(i);
...
%  csvwrite_with_headers('cmin_dec9_roots.csv',CMIN(Nt-4560:Nt),heads)
