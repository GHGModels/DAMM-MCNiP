function [] = bulk2_CN(rootDOC,deep,deep2,filename,filename2,path);
...
for i = 1:Nt
            ...
            DONmult = DOCmult/CN_s; %increase CN from CN_s to 10
      ...
      %Resource Allocation
            ...
            %Dissolved C&N pools
            DOC(i+1,j) = DOC(i,j) + dt * (DOC_input*DOCmult + DECOMPc(i,j) + DEATH_C(i,j)*(1-MIC_to_SOC) + (CN_enz/(1+CN_enz)).*ELOSS(i,j) - UPT_C(i,j));
            DON(i+1,j) = DON(i,j) + dt * (DON_input*DOCmult + DECOM_N(i,j) + DEATH_N(i,j) * (1-MIC_to_SON) + (1/CN_enz).*ELOSS(i,j)- UPT_N(i,j));
...
%year means
%save('C:\Users\rose\Documents\MATLAB\test.mat');
