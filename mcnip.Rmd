...
soilt <- rep(13.73, length(times.def)) #f$SoilT
```
...
derivs <- function(t, state, pars) {
   with(as.list(c(state, pars)), {
     ...
     km_upt_c <- km_upt_n <- b_upt_c + m_upt_c * temp(t) #not in damm-mcnip
     km_c <- km_n <- km_0 + km_slope * temp(t) #not in damm-mcnip
     cue <- b_cue + m_cue * temp(t) #not in damm-mcnip
...
temp <- approxfun(soilt, method = "linear", rule = 2) #create function for input data
...
sR <- sensRange(func = snap, parms = pars, dist = "unif", sensvar, parRange = parRanges[i,], num = 100)
...
```{r senscomb}
SensComb <- summary(sensRange(func = snap, parms = pars, dist = "unif", sensvar = sensvar, parRange = parRanges, num = 100))
...
Data4<- data.frame(name = c("mic_c", "soc", "mic_c", "soc", "mic_c", "soc", "mic_c", "soc"),
