...
T = rep(20,length(soilM)); %rep(mean(F(:,1)),length(soilM));
soilM = rep(0.22,length(soilM)); %rep(mean(F(:,2)),length(soilM));
...
r_death = 0.00015; %0.00015;%Microbial biomass turnover rate, hours-1
...
for i = 1:Nt
    if i > (Nt-4561/2)
    Litter_N = Litter_C/10;%external N input into SON pool (e.g.leaf litter, FWD) mg N cm-3 soil hour-1
    end
