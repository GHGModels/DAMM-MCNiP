...
#variogram shizz 11/5/2015
library(zoo)
dz <- zoo(df)
dz <- na.approx(dz)

write.csv(dz[3:4560], file = "/Users/rzabramoff/Documents/MATLAB/microbial_model_trunk/sensitivity_analysis/hfGapfilled.csv")

par(mfrow=c(2,2))
acf(dz$scale[3:4560], lag.max = 4560) #gapfilled flux from fmeDMC.Rmd
acf(dz$DAMM, lag.max = 4560)
acf(dz$mcnip, lag.max = 4560)
acf(dz$dmc, lag.max = 4560)
par(mfrow=c(2,2))
pacf(dz$scale[3:4560], lag.max = 4560) #gapfilled flux from fmeDMC.Rmd
pacf(dz$DAMM, lag.max = 4560)
pacf(dz$mcnip, lag.max = 4560)
pacf(dz$dmc, lag.max = 4560)
par(mfrow=c(2,2))
ccf(dz$SoilM, dz$scale[3:4560], type = "covariance", lag.max=4560) #gapfilled flux from fmeDMC.Rmd
ccf(dz$SoilM, dz$DAMM, type = "covariance", lag.max=4560)
ccf(dz$SoilM, dz$mcnip, type = "covariance", lag.max=4560)
ccf(dz$SoilM, dz$dmc, type = "covariance", lag.max=4560)
ccf(dz$SoilM, dz$scale[3:4560], type = "correlation", lag.max=4560) #gapfilled flux from fmeDMC.Rmd
ccf(dz$SoilM, dz$DAMM, type = "correlation", lag.max=4560)
ccf(dz$SoilM, dz$mcnip, type = "correlation", lag.max=4560)
ccf(dz$SoilM, dz$dmc, type = "correlation", lag.max=4560)
